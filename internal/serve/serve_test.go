...
type testPipelineExecutorFunc func(context.Context, orchestrator.ExecSpec) (*orchestrator.ExecResult, error)

func (f testPipelineExecutorFunc) Name() string { return "test" }

func (f testPipelineExecutorFunc) Execute(ctx context.Context, spec orchestrator.ExecSpec, _ func(string, int)) (*orchestrator.ExecResult, error) {
	return f(ctx, spec)
}

func TestOrchestratorPipelineExecutePersistsNodeOutput(t *testing.T) {
	bc := NewBroadcaster()
	ctrl := control.New(control.Options{Sink: bc})
	srv := httptest.NewServer(New(ctrl, bc, config.ServeConfig{}).Handler())
	defer srv.Close()

	oldExec := orchestrator.GetExecutorForTest(orchestrator.ExecutorReasonix)
	defer orchestrator.SetExecutorForTest(orchestrator.ExecutorReasonix, oldExec)

	var seenMode string
	var seenModel string
	orchestrator.SetExecutorForTest(orchestrator.ExecutorReasonix, testPipelineExecutorFunc(func(_ context.Context, spec orchestrator.ExecSpec) (*orchestrator.ExecResult, error) {
		seenMode = spec.Mode
		seenModel = spec.ModelRef
		return &orchestrator.ExecResult{FinalText: "stub output", RawStdout: "stub output"}, nil
	}))

	payload := `{"name":"t","nodes":[{"id":"n1","name":"执行者","type":"executor","model":"deepseek-flash","mode":"run","role":"执行","executor":"reasonix","skill":"","agent":"","x":0,"y":0}],"edges":[]}`
	resp, err := http.Post(srv.URL+"/orchestrator/api/pipelines", "application/json", strings.NewReader(payload))
	if err != nil {
		t.Fatal(err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		t.Fatalf("create pipeline status = %d, body = %s", resp.StatusCode, string(body))
	}
	var pipe struct {
		ID string `json:"id"`
	}
	...
	resp2, err := http.Post(srv.URL+"/orchestrator/api/pipelines/"+pipe.ID+"/execute", "application/json", strings.NewReader(`{"task":"say hi"}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	if resp2.StatusCode != http.StatusOK {
		...
		t.Fatalf("execute pipeline status = %d, body = %s", resp2.StatusCode, string(body))
	}
	var run struct {
		ID string `json:"id"`
	}
	...
	var runResp struct {
		...
		NodeStates map[string]struct {
			Status string `json:"status"`
			Output string `json:"output"`
		} `json:"nodeStates"`
	}
	for time.Now().Before(deadline) {
		resp3, err := http.Get(srv.URL + "/orchestrator/api/runs/" + run.ID)
		if err != nil {
			t.Fatal(err)
		}
		if err := json.NewDecoder(resp3.Body).Decode(&runResp); err != nil {
			resp3.Body.Close()
			t.Fatal(err)
		}
		resp3.Body.Close()
	...
	if runResp.Status != "complete" {
		t.Fatalf("run status = %q, want complete", runResp.Status)
	}
	state, ok := runResp.NodeStates["n1"]
	if !ok {
		t.Fatalf("node state n1 missing: %+v", runResp.NodeStates)
	}
	if state.Status != "complete" {
		t.Fatalf("node status = %q, want complete", state.Status)
	}
	if state.Output != "stub output" {
		t.Fatalf("node output = %q, want stub output", state.Output)
	}
	if seenMode != "run" {
		t.Fatalf("executor saw mode = %q, want run", seenMode)
	}
	if seenModel == "" {
		t.Fatal("executor saw empty model ref")
	}
}
...
func TestOrchestratorIndexContainsStewardHome(t *testing.T) {
	...
	body, _ := io.ReadAll(resp.Body)
	html := string(body)
	for _, want := range []string{"多智能体管家", "hero-input", "hero-send", "hero__chip", "把任务交给我", "run-dash", "rd-nodes", "rd-stop"} {
...
func TestOrchestratorNodeTypesExposeExecutorSpecificModels(t *testing.T) {
	...
	resp, err := http.Get(srv.URL + "/orchestrator/api/nodes/types")
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		t.Fatalf("node types status = %d, body = %s", resp.StatusCode, string(body))
	}
	var types []struct {
		Type             string              `json:"type"`
		ModelsByExecutor map[string][]string `json:"modelsByExecutor"`
	}
	...
	for _, item := range types {
		if item.Type == "executor" {
			executorModels = item.ModelsByExecutor
			break
		}
	}
...
func TestServeIndexPagePassesLanguagePreferenceToClient(t *testing.T) {
	...
	// On Windows os.UserConfigDir() reads %AppData%; without this the test
	// would use the real user's config.toml instead of the temp one.
...
func TestSessionsLimitReturnsNewestEntries(t *testing.T) {
	dir := t.TempDir()
	paths := []string{
		filepath.Join(dir, "old.jsonl"),
		filepath.Join(dir, "middle.jsonl"),
		filepath.Join(dir, "new.jsonl"),
	}
	for i, path := range paths {
		sess := agent.NewSession("system")
		sess.Add(provider.Message{Role: provider.RoleUser, Content: filepath.Base(path)})
	...
	ctrl := control.New(control.Options{SessionDir: dir, SessionPath: paths[2], Label: "test"})
	bc := NewBroadcaster()
	srv := New(ctrl, bc, config.ServeConfig{})
	w := httptest.NewRecorder()
	srv.Handler().ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/sessions?limit=2", nil))
	...
	if len(entries) != 2 {
		t.Fatalf("limited session count = %d, want 2: %s", len(entries), w.Body.String())
	}
	if entries[0].Name != "new" || entries[1].Name != "middle" {
		t.Fatalf("sessions are not newest-first: %+v", entries)
	}
}
