...
type chatTUI struct {
	...
	// memoryForm is the interactive "/remember" wizard (nil when closed). While
	// set, keys route to the form and the textarea captures field values.
...
func (m chatTUI) update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case "enter":
			if m.memoryForm != nil {
				return m.handleMemoryFormEnter()
			}
			if m.state == tuiRunning {
