...
const builtinDesignPatternValidatorBody = `This skill is INLINED — runs in the parent loop. Validates and translates design pattern names using the internal/designpattern/ packages.

## Available operations

1. **Lookup a pattern** — resolves English, Chinese, or alias to the canonical GoF pattern definition.
2. **Translate** — convert between Chinese and English design pattern names.
3. **Convert case** — transform a pattern name to PascalCase, camelCase, snake_case, kebab-case, or UPPER_CASE via the converter package.
4. **List patterns** — show all 23 GoF patterns grouped by category (creational/structural/behavioural).
5. **Semantic analysis** — analyse code snippets to check correct design pattern implementation (requires an LLM client implementing the semantic.Client interface).

## Usage

Call the relevant Go API:
- For pattern lookup / matching: use reasonix/internal/designpattern/matcher.NewRegistry()
- For name conversion: use reasonix/internal/designpattern/converter.New(reg)
- For LLM-based analysis: use reasonix/internal/designpattern/semantic with your Client implementation
- Error handling: errorcode.KindOf(err) returns the error category

## Examples

Lookup a pattern: NewRegistry().Lookup("单例模式") → MatchResult{Pattern.English: "Singleton"}
Convert to snake_case: converter.New(reg).ConvertCase("Factory Method", SnakeCase) → "factory_method"
Analyse code: semantic.Analyse(ctx, client, pattern, codeSnippet, cfg)

## Testing

All packages have test coverage under internal/designpattern/*/.
Run: go test ./internal/designpattern/...`
...
func builtinSkills() []Skill {
	...
	base := []Skill{
		...
		{
			Name:        "designpattern-validator",
			Description: "Validate and translate design pattern names — lookup GoF patterns by English/Chinese/alias, convert between naming conventions, and analyse code for correct pattern usage via the internal/designpattern packages.",
			Body:        builtinDesignPatternValidatorBody,
			Scope:       ScopeBuiltin,
			Path:        "(builtin)",
			RunAs:       RunInline,
			Triggers:    []string{"design pattern", "pattern validator", "validate pattern", "设计模式", "模式验证", "模式翻译", "GoF", "pattern lookup"},
			AutoUse:     "suggest",
		},
	}
