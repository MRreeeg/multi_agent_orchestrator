...
const (
	...
	GuardianAssessment
	// PipelineNodeStart signals a pipeline node began execution. Text carries the
	// node label. Appended last to keep the Kind values before it wire-stable.
	PipelineNodeStart
	// PipelineNodeDone signals a pipeline node completed successfully. Text carries
	// the node label, Detail carries the output preview. Appended last.
	PipelineNodeDone
	// PipelineNodeFailed signals a pipeline node failed. Text carries the node
	// label, Detail carries the error. Appended last.
	PipelineNodeFailed
	// PipelineNodeRuntime signals serve runtime status update for a node.
	// Text carries the node ID, Detail carries JSON with endpoint/port/status.
	PipelineNodeRuntime
