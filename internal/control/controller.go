...
func (c *Controller) forkNamed(turn int, name string, switchToFork bool) (string, error) {
	...
	sess := agent.NewSession(c.systemPrompt)
	if cloned, ok := sess.CloneWithMessagesIfCompatible(forked); ok {
		sess = cloned
	} else {
...
func (c *Controller) Branch(name string) (string, error) {
	...
	sess := agent.NewSession(c.systemPrompt)
	if cloned, ok := sess.CloneWithMessagesIfCompatible(branched); ok {
		sess = cloned
	} else {
...
// SetWorkspaceRoot updates the workspace root at runtime (used by the
// orchestrator console's directory picker). It also updates the system prompt's
// "Current workspace:" line so the model sees the new directory.
func (c *Controller) SetWorkspaceRoot(dir string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.workspaceRoot = dir
	// Update the "Current workspace:" line in the system prompt
	if idx := strings.Index(c.systemPrompt, "Current workspace: "); idx >= 0 {
		// Find the end of the line
		endIdx := strings.Index(c.systemPrompt[idx:], "\n")
		if endIdx < 0 {
			endIdx = len(c.systemPrompt) - idx
		}
		prefix := c.systemPrompt[:idx]
		suffix := c.systemPrompt[idx+endIdx:]
		c.systemPrompt = prefix + "Current workspace: " + strconv.Quote(dir) + suffix
	} else {
		c.systemPrompt += "\n\nCurrent workspace: " + strconv.Quote(dir)
	}
	// Also update the executor's current session system message if it exists
	if c.executor != nil {
		if sess := c.executor.Session(); sess != nil {
			sess.SetSystemPrompt(c.systemPrompt)
		}
	}
}
