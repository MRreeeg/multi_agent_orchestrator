...
func currentSystemPrompt(c *Controller) string {
	if c != nil && c.executor != nil {
		snap := c.executor.Session().Snapshot()
		if len(snap) > 0 && snap[0].Role == provider.RoleSystem {
			return snap[0].Content
		}
	}
	if c != nil {
...
func adoptableMessages(systemPrompt string, msgs []provider.Message) []provider.Message {
	if len(msgs) == 0 {
		return nil
	}
	...
	return append([]provider.Message{{Role: provider.RoleSystem, Content: systemPrompt}}, out...)
}
...
func (c *Controller) AdoptHistory(msgs []provider.Message, path string) {
	if len(msgs) > 0 {
		msgs = adoptableMessages(currentSystemPrompt(c), msgs)
		if path != "" {
		...
		c.Resume(agent.NewSession(currentSystemPrompt(c)).CloneWithMessages(msgs), path)
	} else if path != "" {
